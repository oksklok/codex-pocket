...
4. Each changed machine is staged in `.pocket-staging-<bundle>` beside its install, extracted from an archive that travels on stdin, installed with `npm ci` only when `dsh/package-lock.json` changed, and verified by SHA-256 (`--verify`) and a load probe (`--probe`).
5. Activation is idle-only and verified. It writes a `.pocket-deploying` maintenance marker (so an attach client refuses to launch from a half-swapped install), asks the durable runtime to stop through the control endpoint, snapshots the current files to `.pocket-previous`, replaces them, then verifies hashes and the load probe again. Any failure restores `.pocket-previous` automatically. Files are chmod-ed individually; no wildcard chmod and no failure-masking `|| true` remain. An idle state is never inferred: only an explicit idle answer or a genuinely unreachable endpoint permits the swap, while a timeout, a malformed answer, a busy runtime or an install with no control endpoint is treated as not-idle and left pending.
6. The gateway is restarted only after every machine it would attach to re-reads as the protocol the built image speaks, and only while no gateway task is active.
