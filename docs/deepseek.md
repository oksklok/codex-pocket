...
`tests/dsh-lifecycle.test.mjs` starts a real isolated DSH runtime against a local fake Messages endpoint, begins a running turn, kills the gateway carrier process, reattaches a new carrier, and verifies the same runtime owner and turn continue with no duplicate input. It also verifies explicit Stop interrupts the intended turn, that a racing runtime cannot steal the home, and that an unanswered structured question stays pending across the disconnect and is replayed with its original request identity. The checks skip when `npm ci --prefix dsh` has not installed the pinned runtime. Windows named-pipe attach and PowerShell distribution are implemented but were not exercised on an isolated Windows host.
...
The execution-side adapter and the gateway are deployed together with one command, `node scripts/deploy.mjs` (`npm run deploy` where npm is available), run from the repository checkout on the gateway host. It reuses the existing machine settings, SSH aliases and `dshPath` values, stages each adapter update and activates it only when that machine's runtime is idle, installs dependencies only when `dsh/package-lock.json` changed, and reports busy or offline machines as pending so the same command can be rerun. It also refuses to activate a gateway whose protocol the execution adapters do not speak. A gateway/UI-only change leaves the machines untouched. **No native-app rebuild is needed.** Existing task data needs no migration.

The first deployment is a one-time cutover from the old gateway-owned stdio lifecycle: the old adapter process is a child of the gateway and dies with it, so its in-flight turn is still interrupted by the gateway restart. Update the execution adapters and the gateway while no DSH turn is active (the deploy command defers the gateway switch while a turn is running), then restart the gateway once. Every runtime started after that is durable, and later gateway restarts no longer stop accepted DSH work.
