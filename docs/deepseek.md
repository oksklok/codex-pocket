...
Flags: `--dry-run`, `--gateway-only`, `--adapters-only`, `--machine <name-or-ssh>`, `--settings <path>`, `--confirm-idle`, `--allow-protocol-change`, `--rollback`.
