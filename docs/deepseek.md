...
### Durable-runtime checks (2026-09-20)

`tests/dsh-lifecycle.test.mjs` starts a real isolated DSH runtime against a local fake Messages endpoint, begins a running turn, kills the gateway carrier process, reattaches a new carrier, and verifies the same runtime owner and turn continue with no duplicate input. It also verifies explicit Stop interrupts the intended turn, that a racing runtime cannot steal the home, that an unanswered structured question stays pending across the disconnect and is replayed with its original request identity, and that a status check and an idle-only stop leave the attached gateway and its pending question untouched. `tests/deploy.test.mjs` drives the generated POSIX stage/activation/rollback shell against fake installations: a good update replaces the files and retains `.pocket-previous`, a post-activation failure restores them, a busy runtime refuses without mutating anything, and a rollback never deletes the copy it restores from. All three files skip when `npm ci --prefix dsh` has not installed the pinned runtime. `tests/router.test.mjs` drives the correlation state machine directly: a delayed reply to a superseded connection cannot satisfy a new connection that reused the id, a superseded connection cannot submit work, pending questions transfer with their identity, and a draining runtime rejects new work. `tests/deploy.test.mjs` drives the generated POSIX stage/activation/rollback shell against fake installs (loadable staging with reused dependencies, a mid-activation failure that restores, a busy refusal, and a manifest-less legacy rollback), checks that Windows scripts travel as files rather than command-line arguments, and confirms `--stop` returns only after the owner and child exit. Windows named-pipe attach, activation and rollback are implemented but were not exercised on an isolated Windows host.
