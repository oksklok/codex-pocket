...
Do not add automated tests or test harnesses, with one narrow exception: machine-side lifecycle and safety invariants that cannot be validated by hand — the DSH home cold start and ownership lock — are protected by the small Node built-in checks in `test/` (`npm test`) using disposable homes and isolated processes, never a live session or credential. Validate every other changed behavior directly: run the gateway or CLI, exercise the affected flow (including `scripts/deploy.mjs` in a disposable environment when deployment changes), and record exactly what you ran and observed. The read-only smoke check above is an example of that direct validation. Preserve operational safeguards such as health checks, ownership checks and rollback verification.
